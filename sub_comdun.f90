...
!!$c       dv(b,comp,dx)  - displ deriv
  ...
  IMPLICIT none
      REAL(kind(1.d0))  :: (A-H,O-Z)
      real(kind(1.d0))   ::  nu, x1, x2, x3, a, beta, v, dv
      real(kind(1.d0)),parameter :: tol=1e-15
      real(kind(1.d0)),dimension(:,:) :: v(3,3),vc(3,3)
      real(kind(1.d0)),dimension(:,:,:) ::  dv(3,3,3),dvc(3,3,3)
      ...
      if(cb.lt.tol.and.y3.lt.tol)then
	cb=cb+tol
	y3=y3+tol
	end if
